...
# If the buildNumber is empty, call the update buildNumber function
...
foreach ($os in $targets) {
    ...
    foreach ($_arch in $archs) {
        ...
        $cli_filename = "{0}/mrbuild-{1}-{2}-{3}" -f $BasePath, $os, $_arch, $BuildNumber
        $inttest_filename = "{0}/mrbuild-{1}-inttest-{2}-{3}" -f $BasePath, $os, $_arch, $BuildNumber
        ...
        $cmd = 'go build -ldflags "-X github.com/amido/mrbuild/cmd.version={0}" -o {1}' -f
                    $BuildNumber,
                    $cli_filename
        ...
        $cmd = 'go test -ldflags "-X github.com/amido/mrbuild/testing/integration.version={0}" -tags integration -o {1} -c github.com/amido/mrbuild/testing/integration/...' -f
                    $BuildNumber,
                    $inttest_filename
